...
items={}
info={'my_id':None, 'my_ip':'192.168.1.1:5000', 'prev_id':None, 'next_id':None, 'prev_ip':None, 'next_ip':None}
...
result = hashlib.sha1(info['my_ip'].encode())
info['my_id'] = int(result.hexdigest(), 16) % 2**160
info['prev_id'] = info['my_id']
info['next_id'] = info['my_id']
info['prev_ip'] = info['my_ip']
info['next_ip'] = info['my_ip']
...
# Route for updating keys in case another node departs
		...
		if key_in_area(key, last_id, info['my_id']):
			...
			if linearizability:
				if info['next_ip'] != info['my_ip']:
					...
					# If only bootstrap node is in Chord
...
def replica():
	if request.method == 'POST':
		...
		if info['my_ip'] == src_ip and replica_id !=-1:
			...
			sent = {}
			for key, value in items.items():
				if key_in_area(key, info['prev_id'], info['my_id']):
					sent[key] = value
...
if __name__ == '__main__':
	...
	app.run(host='192.168.1.1',debug = False)
